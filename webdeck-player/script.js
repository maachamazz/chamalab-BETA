...
*/
/* ★ ここに自分のYouTube再生リストを設定 ★
   1. YouTubeで再生リストを作成(公開 or 限定公開にする)
   2. 再生リストのURLの「list=」より後ろの文字列をコピー
      例: https://www.youtube.com/playlist?list=PLxxxx → PLxxxx の部分
   3. 下の形式で書き換え・追加する                              */
var myPlaylists = {
    "BGM": 'PLZyqOyXxaVETqpHhT_c5GPmAPzhJpJ5K7',   // ←サンプル(90s HITS)。自分のIDに変えてね
    //"作業用": 'ここに再生リストID',
};
...
// ===== CHAMALAB: 親ページ(Welcome ENTER)からの再生指示を受け取る =====
var chamalabWantPlay = false;
window.addEventListener('message', function(e){
    if (e.data === 'chamalab-autoplay') {
        if (typeof player !== 'undefined' && player && player.playVideo) {
            try { player.playVideo(); } catch(err) { chamalabWantPlay = true; }
        } else {
            chamalabWantPlay = true;
        }
    }
});
